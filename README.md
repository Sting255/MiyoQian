...
终端会打印地址，默认 **http://127.0.0.1:5890**（端口不可用时会自动换，以终端打印的为准）。
...
- **Web 控制台打不开？** 看终端打印的实际地址；端口不可用会自动换。要是日志报 `[WinError 10013]`，那是**端口被系统保留**（开了 Hyper-V / WSL 常见，重启后保留区间还会变），见 [docs/faq.md](docs/faq.md#web-控制台打不开怎么办)。
