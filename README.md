...
- [保持运行 / 开机自启](#保持运行--开机自启)
- [Web 控制台说明](#web-控制台说明)
- [IP 防护（境外 IP 时暂停签到）](#ip-防护境外-ip-时暂停签到)
- [配置说明](#配置说明)
- [米游社商品兑换](#米游社商品兑换)
- [米游币任务](#米游币任务)
- [推送设置](#推送设置)
- [命令行用法](#命令行用法)
- [推荐使用流程](#推荐使用流程)
- [其它运行方式（Docker / 云函数 / GitHub Actions）](#其它运行方式docker--云函数--github-actions)
- [常见问题 FAQ](#常见问题-faq)
- [开发与测试](#开发与测试)
- [注意事项](#注意事项)
- [跟上原项目的更新](#跟上原项目的更新)
...
- **本地验证码识别** —— 用本机 Chrome + CDP + 一个轻量视觉模型直接解极验九宫格，不依赖第三方打码服务（网页上有「环境自检」按钮可先验证环境），打码狗是另一个可选渠道 —— 两个渠道是**二选一**：程序只取配置里第一个启用的渠道，本地识别失败不会自动切到打码狗。
- **出口 IP 防护** —— IP 在境外时暂停签到，避免异地登录风控；兑换计划在**创建/保存时**（也就是计划线程起来的那一刻）查一次出口 IP，不做阻塞等待，所以不会占用开抢时间；那一刻在境外的计划会被直接跳过。
...
米游签是一款小而美，精致便捷的米游社每日签到工具。你只需要登录一次（扫码，或手机号 + 短信验证码），之后就可以在本地 Web 控制台里管理账号、选择任务、设置每天自动执行签到任务。
...
需要 Python 3.11，项目用 [uv](https://docs.astral.sh/uv/) 管理环境和依赖。
...
> 装得慢可以换清华源：`uv sync --index-url https://pypi.tuna.tsinghua.edu.cn/simple`
...
终端会打印控制台地址，默认是 **http://127.0.0.1:5890**。如果这个端口被占用，程序会**自动往后试最多 30 个端口**，以终端里显示的实际地址为准。

### 3. 扫码登录账号

1. 在「账号」区域点 **添加**
2. 把账号名改成 `大号`、`小号` 之类方便识别的名字
3. 点账号卡片里的 **登录**，会先让你选登录方式：
   - **扫码登录** —— 弹出二维码
   - **验证码登录** —— 填手机号、收短信验证码（手边没有第二台设备时用这个）
4. 扫码的话用米游社 APP：**我的 → 左上角扫一扫**

登录成功后账号卡片会显示 UID。凭证写在 `data/credentials.yaml`，**不在 `config.yaml` 里** —— 所以 `config.yaml` 可以放心分享，凭证文件绝对不要外传。

### 4. 选任务、手动跑一次

在「任务配置」里选择要跑的内容：游戏社区签到、具体游戏、云游戏签到、米游币任务。

> 第一次建议**只开游戏社区签到**，跑通再加别的。米游币任务更容易碰到验证码和风控。

点右上角 **立即执行**，右侧日志区会实时显示过程。提示「今日已签到」就说明账号和配置都正常。
...
1. 勾选 **启用每日自动执行**
2. 设置执行时间（例如 `09:00`）
3. 设置随机波动分钟（例如 `30`，表示在 09:00~09:30 之间随机挑一个时刻执行 —— 别天天卡整点，容易被风控）
4. 需要的话勾上 **启动后执行一次**

**自动执行要求程序保持运行**，所以接着看下一节。

## 保持运行 / 开机自启

关闭终端、电脑关机或休眠后，自动任务都不会执行。要让它每天自己跑，就得让进程常驻。

### Windows：计划任务 + 看门狗

仓库里带了两个现成脚本：

- `start-miyoqian.ps1` —— 启动服务，并每 60 秒检查一次端口；服务挂了会自动重新拉起（看门狗）。
- `start-miyoqian-hidden.vbs` —— 用 `wscript.exe`（GUI 子系统）以**隐藏窗口**方式调用上面的脚本，不会弹控制台窗口。

注册一个「登录时自动启动」的计划任务（**注册这一步需要管理员权限**）：
...
> ⚠️ **这两个坑都踩过，照抄时注意：**
>
> 1. **任务动作别直接写 `powershell.exe`。** 它是控制台程序，会弹出一个终端窗口；而且 `-WindowStyle Hidden` 也挡不住 —— 它启动的 `uv.exe` / `python.exe` 同样是控制台程序，会继承并让那个窗口重新显示出来。走 `wscript.exe` + `.vbs`（窗口样式 0）才真正无窗口。
> 2. **一定要加 `-DontStopOnIdleEnd`。** 计划任务的默认值是「空闲 10 分钟后，一旦空闲状态结束就停止任务」（`StopOnIdleEnd = True`），而**停止任务会连带杀掉整个子进程树**（看门狗和服务一起没）。表现是：你离开电脑一会儿、回来动一下鼠标，服务就凭空消失，而日志里**没有任何报错**。检查命令：`Get-ScheduledTask MiyoQianWebUI | Select-Object -ExpandProperty Settings`。

### Linux / macOS

用 `systemd --user` 服务或 `nohup` + cron 让它常驻即可，`start.sh` 会处理好依赖安装。只要保证进程一直活着。

> 注意：**「本地验证码识别」需要有 Chrome/Edge 的机器**。纯服务器（没有浏览器）请改用打码狗，或者干脆关掉米游币任务 —— 见「[配置说明](#配置说明)」里的验证码一节。
...
| 账号 | 添加账号、扫码登录或短信验证码登录、刷新凭证、改账号名（铅笔图标）、删除账号、设置该账号的独立任务 |
| 任务配置 | 开关游戏社区签到、云游戏签到和米游币任务（所有账号的默认值） |
| 设备指纹 | 查看和更换机型、设备 ID、设备 FP |
| 每日调度 | 设置每天什么时候自动运行，以及账号之间的随机等待 |
| 推送通道 | 设置任务完成后的通知方式；面板里的「测试推送」可以先用示例结果验证渠道通不通 |
| 验证码识别 | 配置本地识别 / 打码狗渠道；「环境自检（本地识别）」按钮当场验证本机浏览器环境 |
| IP 防护 | 境外 IP 时暂停签到，恢复后自动继续；「立即检测出口 IP」看两条链路的实时结果 |
| 兑换 | 浏览和筛选商品（全部 / 已售罄 / 可兑换）、刷新商品、自动兑换开关、兑换计划管理，以及状态条（自动兑换 / 下次兑换 / 商品数量 / 计划数量） |
| 日志 | 查看本次启动后的运行记录 |
...
| 子命令 | 作用 |
| --- | --- |
| `init` | 生成默认配置（`--force` 覆盖已存在的配置） |
| `login` | 米游社 APP 扫码登录并写入凭证。`--account 名字`（默认 `main`）、`--timeout 秒`（默认 120）、`--no-image`（不保存二维码图片） |
| `run` | 执行签到任务，参数见下表 |
| `serve` | 启动常驻 Web 控制台和每日调度器，`--host` / `--port` 覆盖配置文件里的值 |
| `show` | 打印当前启用配置的摘要 |
...
| --- | --- |
| `--account 名字` | 只执行指定账号 |
| `--games-only` | 只执行游戏社区签到和云游戏签到 |
| `--bbs-only` | 只执行米游币社区任务（不能和 `--games-only` 一起用） |
| `--game 游戏名` | 只执行指定游戏，可重复传：`--game genshin --game starrail` |

常用例子：

```powershell
uv run python main.py init                        # 生成配置文件
uv run python main.py login --account 大号         # 扫码登录指定账号
uv run python main.py run                         # 执行全部账号的全部任务
uv run python main.py run --games-only            # 只跑游戏社区 + 云游戏签到
uv run python main.py run --bbs-only              # 只跑米游币任务
uv run python main.py run --account 小号           # 只跑一个账号
uv run python main.py serve --port 5891           # 换端口启动 Web 控制台
uv run python main.py show                        # 查看配置摘要
```

`-c` / `--config` 可以指定其它配置文件（默认 `config.yaml`）；`-v` / `--version` 打印版本。

### 退出码

`run` 会**按任务结果返回退出码**：成功 `0`、失败 `1`。cron、计划任务、CI 都可以直接用它判断要不要告警。
